...
MOFAobjectNew <- readRDS("MOFA2_CLL.rds")
...
samples_metadata(MOFAobjectNew) <- CLL_metadata
```
...
plot_variance_explained(MOFAobjectNew, max_r2=15)
```
...
plot_variance_explained(MOFAobjectNew, plot_total = TRUE)[[2]]
```
...
correlate_factors_with_covariates(MOFAobjectNew, 
  covariates = c("Gender","died","age"), 
  plot="log_pval"
)
```
...
plot_top_weights(MOFAobjectNew,
 view = "Mutations",
 factor = 1,
 nfeatures = 10,     # Top number of features to highlight
 scale = TRUE           # Scale weights from -1 to 1
...
plot_data_scatter(MOFAobjectNew, 
  view = "mRNA",
  factor = 1,  
  features = 4,
  sign = "positive",
...
plot_data_heatmap(MOFAobjectNew, 
  view = "mRNA",
  factor = 1,  
  features = 25,
  cluster_rows = FALSE, cluster_cols = FALSE,
